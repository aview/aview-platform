package edu.amrita.aview.core.whiteboard.objectHandle.decorators
{
	...
	public class AlignmentDecorator implements IDecorator
	{
		...
		public function updatePosition(allObject:Array, selectedObjects:Array, movedObjects:Array, drawingCanvas:Sprite):void
		{
			...
			for each (var other:Object in allObject)
			{
				if (other == o)
					continue;
				if (Math.abs(o.x - other.x) < 1)
					drawVerticalLine(other.x, drawingCanvas);
				if (Math.abs(o.y - other.y) < 1)
					drawHorizontalLine(other.y, drawingCanvas);
				if (Math.abs((o.x + o.width) - (other.x + other.width)) < 1)
					drawVerticalLine(other.x + other.width, drawingCanvas);
				if (Math.abs((o.y + o.height) - (other.y + other.height)) < 1)
					drawHorizontalLine(other.y + other.height, drawingCanvas);
				
				if (Math.abs(o.x - (other.width + other.x)) < 1)
					drawVerticalLine(o.x, drawingCanvas);
				if (Math.abs(o.y - (other.y + other.height)) < 1)
					drawHorizontalLine(o.y, drawingCanvas);
				if (Math.abs((o.x + o.width) - other.x) < 1)
					drawVerticalLine(o.x + o.width, drawingCanvas);
				if (Math.abs((o.y + o.height) - other.y) < 1)
					drawHorizontalLine(o.y + o.height, drawingCanvas);
