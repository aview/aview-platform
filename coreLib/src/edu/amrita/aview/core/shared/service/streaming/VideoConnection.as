...
package edu.amrita.aview.core.shared.service.streaming
{
	...
	public class VideoConnection
	{
		...
		private var cameraIndexsetLocal:int=1;
		...
		applicationType::mobile{
			public var selectedViewerAudioLevelLst:ArrayList = new ArrayList();
		}
		...
		public function createConnection():void
		{
			...
			ncVideo.addEventListener(MediaServerStatusEvent.CODE_COULD_NOT_RECONNECT, maxRetrysReached);
			ncVideo.initialize();
		}
...
//		verbosity?
		...
		public function stoppedStream(streamName:String, disconnectedDuringRetrys:Boolean):void
		{
			if (ClassroomContext.isModerator && FlexGlobals.topLevelApplication.mainApp.mainContainerComp.classroomComp.recorder.isRecording)
			{
				...
				if (ClassroomContext.currentPresenterName == streamName)
				{
					...
					FlexGlobals.topLevelApplication.mainApp.mainContainerComp.classroomComp.recorder.presenterVideoRecorder.addEndtime(FlexGlobals.topLevelApplication.mainApp.mainContainerComp.classroomComp.recorder.getCentralTime(), streamName);
				}
		...
		public function areAllStreamsInPublishedState():Boolean
		{
			...
			for (var i:int=0; i < streamObjects.length; i++)
			{
				...
				if (tempStream.serverType == "FMS_VIDEO_PRESENTER")
				{
		...
		public var isMaxRetrysReached:Boolean = false
		 ...
		 * 
		 */
		private function maxRetrysReached(event:MediaServerStatusEvent):void
		{
			isMaxRetrysReached = true;
		}
		...
		private function netStatusHandler(event:MediaServerStatusEvent, calledFromTimeout:Boolean=false):void
		{
			 ...
			 * During such inconsistant status, it's better to set the status as 'NetConnection.Connect.NetworkChange' and make it wait for some time until connection status become consistent
			*/
			if (netConnectionStatus == MediaServerStatusEvent.CODE_NET_STATUS_CLOSED && ncVideo.netConnection && ncVideo.isConnected())
			{
				if (Log.isInfo()) log.info("netStatusHandler:- Inconstant Connection status. Setting the status to NetworkChange. ipFMS :" + ipFMS + ", netConnectionStatus :" + netConnectionStatus + ". NetConnection connected:" + ncVideo.isConnected() + ", ncVideo.connectionRetrys:" + ncVideo.connectionRetrys + ":");
				netConnectionStatus=MediaServerStatusEvent.CODE_NET_STATUS_CHANGE;
			}
...
//			if(Log.isDebug()) log.debug("netConnectionStatus"+netConnectionStatus);
			switch (netConnectionStatus)
			{
				...
				case MediaServerStatusEvent.CODE_NET_STATUS_SUCCESS:
					...
					if (ncVideo.connectionRetrys != 0)
					{
						applicationType::DesktopWeb{
							...
							if (ClassroomContext.isModerator && FlexGlobals.topLevelApplication.mainApp.mainContainerComp.classroomComp.recorder && FlexGlobals.topLevelApplication.mainApp.mainContainerComp.classroomComp.recorder.isRecording)
							{
								if (FlexGlobals.topLevelApplication.mainApp.mainContainerComp.classroomComp.recorder.presenterVideoRecorder.currentrecordingStream == userName)
								{
									...
									FlexGlobals.topLevelApplication.mainApp.mainContainerComp.classroomComp.recorder.presenterVideoRecorder.addEndtime(recordingTimeBeforReconnection, userName);
								}
								if (FlexGlobals.topLevelApplication.mainApp.mainContainerComp.classroomComp.recorder.viewerVideoRecorder.currentrecordingStream == userName + "_VIEWER")
								{
				...
				case MediaServerStatusEvent.CODE_NET_STATUS_CHANGE:
					processConnectionChangeHandler();
					break;
		...
		private function processConnectionChangeHandler() : void
		{
			applicationType::DesktopWeb{
				AuditContext.userAction.connectionFailEventLog("Video Module", videoFMSURL);
			}
		...
		private function startVideoPublishingProcess(videoStream:VideoStream):void
		{
			...
			applicationType::mobile{
				...
				}else if (this.videoCodec == VideoConnection.CODEC_SORENSON || this.videoCodec == VideoConnection.CODEC_H264){
					if (Log.isDebug()) log.debug("startVideoPublishingProcess:- ipFMS :" + ipFMS + ", with codec :" + this.videoCodec + ": stream :" + videoStream.streamName);
					publishEncodedVideo(videoStream);
				}
			}
			applicationType::desktop
			{
				if (Capabilities.os.toLowerCase().indexOf("win") > -1)
				{
					...
					{
						if (videoStream.data.userStatus == 1 || (videoStream.data.streamName==ClassroomContext.userVO.userName && FlexGlobals.topLevelApplication.mainApp.mainContainerComp.classroomComp.classroomContextObj.userRole == Constants.VIEWER_ROLE && ClassroomContext.isModerator))
						{
		...
		private function publishEncodedVideo(videoStream:VideoStream):void
		{
			...
			{
				applicationType::mobile{
					microPhoneObj = Microphone.getMicrophone(this.micIndex);
					microPhoneObj.codec = SoundCodec.PCMA;
				}
				applicationType::DesktopWeb{
				...
				if (!this.isAudioOnlyOption)
				{
					...
					//Temporary fix: In multi bitrate class, teacher low bandwidth video is not published.
					if (videoStream.data.userStatus == 1 || (ClassroomContext.aviewClass.isMultiBitrate == "Y" && ClassroomContext.userVO.role==Constants.TEACHER_TYPE && videoStream.data.userStatus == 1))
					{
			...
			{
				if (!this.isAudioOnlyOption)	
				if (Log.isDebug()) log.debug("publishEncodedVideo:- ipFMS :" + ipFMS +", Encoder :"+ ClassroomContext.aviewClass.videoCodec + ", Setting the PUBLISHING_STATE for stream :" + videoStream.streamName +", cameraObj.name :"+cameraObj.name+",cameraObj.width:"+cameraObj.width+",cameraObj.height:"+cameraObj.height+",cameraObj.fps:"+cameraObj.fps+",cameraObj.keyFrameInterval:"+cameraObj.keyFrameInterval+",cameraObj.bandwidth:"+cameraObj.bandwidth+ " and publishing");
				videoStream.streamState=PUBLISHING_STATE;
				videoStream.publish();
		...
		private function killVideoPublishingProcessHandler():void
		{
			...
			try
			{
				for (var i:int=0; i <streamObjects.length; i++)
				{
					...
					{
						if(FlexGlobals.topLevelApplication.mainApp.mainContainerComp.classroomComp.isLocalAudioVideoSettings && FlexGlobals.topLevelApplication.mainApp.mainContainerComp.classroomComp.isAudioOnlyOptionSelected)
					...
					else if (objStream.videoCodec == VideoConnection.CODEC_SORENSON || objStream.videoCodec == VideoConnection.CODEC_H264)
					{
						...
						else
						{
							...
							try
							{
								if (FlexGlobals.topLevelApplication.mainApp.mainContainerComp.classroomComp.processAttachCam && FlexGlobals.topLevelApplication.mainApp.mainContainerComp.classroomComp.attachedCameraToScreenCamera == true)
								{
									FlexGlobals.topLevelApplication.mainApp.mainContainerComp.classroomComp.cameraToScreenCamera(0, "ScreenCamera");
								}
							}catch (err:Error)
							{if(Log.isError()) log.error("Error in killVideoPublishingProcessHandler method:"+ err.getStackTrace());}
						}
		...
		public function stopStream(StreamName:String):void
		{
			...
			for (var i:int=0; i < streamObjects.length; i++)
			{
				if (streamObjects[i].streamName == StreamName && streamObjects[i].streamType == SUBSCRIBE_TYPE)
				{
					...
					applicationType::mobile{
						if(FlexGlobals.topLevelApplication.videoComp.videoStartStopIcon != FlexGlobals.topLevelApplication.videoComp.stopVideoIcon )
						{
							streamObjects[i].stream.attachCamera(null);
						}
					}
		...
		public function checkprogressBar(streamName:String):String{
			var micCamActivityLevel:String="0_0";
			if(!FlexGlobals.topLevelApplication.mainApp.mainContainerComp.classroomComp.isFMLE)
			for (var i:int=0; i < streamObjects.length; i++)
			{
				if (streamObjects[i].streamName == streamName && streamObjects[i].streamType == PUBLISH_TYPE)
				{
					//var cameraLevel:String="0";
					var cameraLevel:String;
					if(!this.isAudioOnlyOption && cameraObj)			
						cameraLevel = cameraObj.activityLevel.toString();
					if(microPhoneObj)
						micCamActivityLevel = microPhoneObj.activityLevel +"_" +cameraLevel;
					//trace("cameraLevel =" +microPhoneObj.activityLevel +"_" +cameraLevel);
					break;
				}	
					
			}
			return micCamActivityLevel;
					//return cameraObj.activityLevel;
		}
		...
		public function startInternalCodecCapture(portNo:int):void
		{
			...
			for (var i:int=0; i < streamObjects.length; i++)
			{
				...
				if (tempStream.portNO == portNo && tempStream.streamState != PUBLISHED_STATE && tempStream.streamState != PUBLISHING_STATE)
				{
					...
					if (!this.isAudioOnlyOption)
					{
						if (tempStream.data.streamNumber == 1 || ClassroomContext.aviewClass.isMultiBitrate == "N")
						{
					...
					if(cameraObj)
					if (Log.isDebug()) log.debug("startInternalCodecCapture:- ipFMS :" + ipFMS + ", portNo :" + tempStream.portNO + " stream name:" + tempStream.streamName + " streamState:" + tempStream.streamState+", cameraObj.name :"+cameraObj.name+",cameraObj.width:"+cameraObj.width+",cameraObj.height:"+cameraObj.height+",cameraObj.fps:"+cameraObj.fps+",cameraObj.keyFrameInterval:"+cameraObj.keyFrameInterval+",cameraObj.bandwidth:"+cameraObj.bandwidth)
...
//																	STREAM_UN_MUTE : STREAM_MUTE;
		 ...
		 * 
		 */
		public function callHideVideo(isHide:Boolean):void{
			for (var i:int=0; i < streamObjects.length; i++)
			{
				var tempStream:VideoStream=streamObjects[i];
				if (tempStream.streamType == PUBLISH_TYPE)
				{
					if(isHide)
						...
						streamObjects[i].stream.attachCamera(cameraObj);
				}
			}
		}
		...
		public function callMuteAudio(isMute:Boolean):void{
			for (var j:int=0; j < streamObjects.length; j++)
			{
				var tempStream:VideoStream=streamObjects[j];
				if (tempStream.streamType == PUBLISH_TYPE)
				{
					if(isMute)
						streamObjects[j].stream.attachAudio(null);
					else
						streamObjects[j].stream.attachAudio(microPhoneObj);	
				}
			}
		...
		public function getLogInActivity(StreamName:String):ArrayCollection{
			for (var i:int=0; i < streamObjects.length; i++)
			{
				var logInActivity:ArrayCollection=new ArrayCollection();
				var logLevel:Object = new Object();
				if (streamObjects[i].streamName == StreamName && streamObjects[i].streamType == SUBSCRIBE_TYPE)
				{
					netInfo=streamObjects[i].stream.info;
					logLevel.cbps = Math.round(netInfo.currentBytesPerSecond);
					logLevel.droppedFrames = Math.round(netInfo.droppedFrames);
					logInActivity.addItem(logLevel);	
					trace("CurrentBytes = " +logLevel.cbps+ " DroppedFrames = " +logLevel.droppedFrames);
					break;
				}		
			}
