...
package edu.amrita.aview.core.shared.service.streaming
{
	...
	public class DynamicStream extends NetStream
	{
		...
		private const PREFERRED_BUFFERLENGTH:Number=5; //trying to keep it around 3 times the QoS Interval so it gets 3 times to update the bitrate
		private const STARTUP_BUFFERLENGTH:Number=2;
		private const EMPTY_BUFFERLENGTH:Number=1;
		
		private const PREFERRED_BUFFERLENGTH_LIVE:Number=1;
	...
	*/
		...
		private function checkLiveQOSAndSwitch():void
		{
			...
			if ((this.bufferLength < _preferredBufferLength / 2) || ((_maxBandwidth < dsPlayList[dsPlayIndex].streams[_curStreamID].rate) && (_maxBandwidth != 0)) || ((this.info.droppedFrames - _previousDroppedFrames) * 1000 / (nowTime - _previousDroppedFramesTime) > this.currentFPS * 0.25))
			{
		...
		private function checkVodQOSAndSwitch():void
		{
			...
			//downscale
			var nowTime:int=getTimer();
			if ((this.bufferLength < _preferredBufferLength) || ((_maxBandwidth < dsPlayList[dsPlayIndex].streams[_curStreamID].rate) && (_maxBandwidth != 0)) || ((this.info.droppedFrames - _previousDroppedFrames) * 1000 / (nowTime - _previousDroppedFramesTime) > this.currentFPS * 0.25))
			{
				var nextStreamID:int=0;
