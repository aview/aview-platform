...
applicationType::DesktopWeb{
	...
	import edu.amrita.aview.core.shared.components.fileManager.FolderCreation;
	import edu.amrita.aview.core.shared.components.fileManager.Upload;
	import edu.amrita.aview.core.shared.components.fileManager.XMLLoadedTree;
...
import edu.amrita.aview.core.shared.components.fileManager.events.DragFinish;
...
applicationType::mobile{
	...
	import edu.amrita.aview.core.shared.components.fileManager.MobileFolderCreation;
...
public function onInitilize():void
{
	if (isSharedLibraryVisible)
	{
		...
		{
			personalArea.includeInLayout=false;
			personalArea.visible=false;
			cpyCan.includeInLayout=false;
			cpyCan.visible=false;
			fileManagerButtonBox.includeInLayout=false;
			fileManagerButtonBox.visible=false;
			applicationType::mobile{
				commonArea.percentWidth=100;
			}
			centralRepXMLList=dummyXML.children();
			var event:ContentOperationEvent=new ContentOperationEvent(ContentOperationEvent.SHAREDFILELIST);
...
private function selectionInCommon(event:Event):void
{
	...
	{
	remoteFilePath=event.currentTarget.selectedItem.@path;
	remoteFileName=event.currentTarget.selectedItem.@label;
	}
...
private function onUploadInitilize(ev:ContentOperationEvent):void
{
	...
	event.isAnimatedFile=ev.isAnimatedFile;
	this.dispatchEvent(event);
...
applicationType::mobile{
	...
	private function instituteFileList(event:Event):void
	{
		...
		commonTree.dataProvider=courseListName;
		if (courseListName.children() != null)
		{
			commonTree.addEventListener(Event.CHANGE, courseFileList);
		}
	}
	/**
	 * @private
	...
	private function courseFileList(event:Event):void
	{
		...
		if (xml.name() == "folder")
		{
			...
			commonTree.dataProvider=classListName;
			var obj:Object={data: xml.@path};
		...
		{
			commonTree.addEventListener(MouseEvent.CLICK, commonFileListClickHandler);
		}
	}
	/**
	 * @private
	...
	{
		commonTree.removeEventListener(MouseEvent.CLICK, commonFileListClickHandler);
	}
	/**
	 * @private
	 * 
	...
	private function commonBackWardNavigate(event:Event):void
	{
		...
		if (fileListName.length.toString() != "0")
		{
			...
			var fileListProvider:XMLListCollection=new XMLListCollection(item.children());
			commonTree.dataProvider=fileListProvider;
