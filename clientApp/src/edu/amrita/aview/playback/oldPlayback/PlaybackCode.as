...
import edu.amrita.aview.playback.oldPlayback.PlaybackLoading;
import edu.amrita.aview.playback.oldPlayback.Playback_Video_fullscreen;
import edu.amrita.aview.playback.oldPlayback.Print2Flash.Point;
...
* This method is called by LMS modules i.e.  edu.amrita.aview.lms.LMS.mxml
...
public function PlaybackWindowResizeHandler():void
{
	...
	{
		document.setScrollPosition(new edu.amrita.aview.documentsharing.Print2Flash.Point(documentCurrentScrollPosX *(documentCurrentScrollPosX/documentMaxScrollPosX) ,documentCurrentScrollPosY*(documentCurrentScrollPosY/documentMaxScrollPosY)));
	} */
...
public function loadSlide(timeValue:Number, count:int):void
{
	...
	if (pptXmlDataArrColl != null)
	{
		...
		if ((pptXmlDataArrColl[count].fileName) == pptFileName)
		{
			...
			//document.setScrollPosition(new edu.amrita.aview.documentsharing.Print2Flash.Point(scrollXvalue,scrollYvalue));	         			   
		...
		else
		{
			if (pptXmlDataArrColl[count].tab != "tab")
			{
				...
				document.source=xmlPath + "RecordFilesAview/Slides/" + pptFileName;
				document.setCurrentPage(parseFloat(pptXmlDataArrColl[count].page));
				document.setScrollPosition(new edu.amrita.aview.playback.oldPlayback.Print2Flash.Point(documentCurrentScrollPosX, documentCurrentScrollPosY));
			}
		}
