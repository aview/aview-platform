...
private function checkFileExistance(filePath:String):void
{
	...
	checkFileExistanceService.url=encodeURI("http://" + serverIPAddress + ":" + ClassroomContext.portWAMP + "/AVScript/Upload/checkFileExistance.php?filePath=" + filePath);
	checkFileExistanceService.addEventListener(ResultEvent.RESULT, checkFileExistanceResultHandler);
	checkFileExistanceService.addEventListener(FaultEvent.FAULT, faultHandler);
	checkFileExistanceService.send();
}
...
private function faultHandler(event:FaultEvent):void
{
	...
	if (usingModule == MobileFileManager.MODULE_DOCUMENT_SHARING)
	{
		dummyXMLForDocument=<dirs><files label="Doucument listing is failed  due to network issue.Please Try Again..." type="fault"/></dirs>;
	}
	else if (usingModule == MobileFileManager.MODULE_3D_SHARING)
	{
		dummyXMLForDocument=<dirs><files label="3D object listing is failed  due to network issue.Please Try Again..." type="fault"/></dirs>;
	}
	treeXmlListDp=dummyXMLForDocument.children();
